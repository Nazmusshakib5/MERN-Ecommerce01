...
const Products = () => {
    ...
    return (
        <div>
            {
                ProductListByRemark===null?(<ProductSkeleton/>): (<div className="section">
                                ...
                                <div>
                                    <ul className="nav nav-pills p-3 justify-content-center mb-3" id="pills-tab"
                                        ...
                                        <li className="nav-item" role="presentation">
                                            <button onClick={async () => {await ProductListByRemarkRequest('new')}} className="nav-link active" id="pills-home-tab" data-bs-toggle="pill"
                                                    data-bs-target="#pills-new" type="button" role="tab"
                                                    aria-controls="pills-home" aria-selected="true">New
                                            </button>
                                        </li>
                                        <li className="nav-item" role="presentation">
                                            <button onClick={async () => {await ProductListByRemarkRequest('trending')}} className="nav-link" id="pills-profile-tab" data-bs-toggle="pill" data-bs-target="#pills-trending" type="button" role="tab"
                                                    aria-controls="pills-profile" aria-selected="false">Trending
                                            </button>
                                        </li>
                                        <li className="nav-item" role="presentation">
                                            <button onClick={async () => {await ProductListByRemarkRequest('popular')}} className="nav-link" id="pills-contact-tab" data-bs-toggle="pill" data-bs-target="#pills-popular" type="button" role="tab"
                                                    aria-controls="pills-contact" aria-selected="false">Popular
                                            </button>
                                        </li>
                                        <li className="nav-item" role="presentation">
                                            <button onClick={async () => {await ProductListByRemarkRequest('top')}} className="nav-link" id="pills-disabled-tab" data-bs-toggle="pill"
                                                    data-bs-target="#pills-top" type="button" role="tab"
                                                    aria-controls="pills-disabled" aria-selected="false">Top
                                            </button>
                                        </li>
                                        <li className="nav-item" role="presentation">
                                            <button onClick={async () => {await ProductListByRemarkRequest('special')}} className="nav-link" id="pills-disabled-tab" data-bs-toggle="pill"
                                                ...
                                                <div className="row">
                                                    {
                                                        ProductListByRemark.map((item,i)=> {
                                                            ...
                                                            if(item['discount']){
                                                                price = <p className="bodyMedium text-secondary my-1"><del>{item['price']}</del><span style={{marginLeft:'10px'}}>{item['discountPrice']}</span></p>
                                                            }
                                                            return (
                                                                <div key={i}
                                                ...
                                                <div className="row">
                                                    {
                                                        ProductListByRemark.map((item, i) => {
                                                            let price = <p
                                                                className="bodyMedium text-secondary my-1">{item['price']}</p>
                                                            if (item['discount']) {
                                                                price = <p className="bodyMedium text-secondary my-1">
                                                                    <del>{item['price']}</del>
                                                                    <span
                                                                        style={{marginLeft: '10px'}}>{item['discountPrice']}</span>
                                                                </p>
                                                            }
                                                            return (
                                                                <div key={i}
